...
$app->post('/deleteWishGroup', function (Request $request, Response $response, array $args) {
    $cnn = new DB();
    ...
    try{
        ...
        $sqlWishGroup = "DELETE FROM `wish_list_groups` 
            WHERE `wish_list_groups`.`id_group` = '{$id_group}' 
            AND `wish_list_groups`.`id_item` = '{$id_item}'";

        $stmt = $cnn->query($sqlWishGroup);

        if (!$stmt) {
            throw new Exception("Ha surgido un error en la consulta 1, intentelo de nuevo", 1); 
        }

        $cnn-> close(); 

        $resp = '{"text": "Item borrado correctamente"}';
...
$app->post('/deleteWishItem', function (Request $request, Response $response, array $args) {
    $cnn = new DB();
    ...
    try{
        ...
        $sqlItems = "DELETE FROM `items` WHERE `items`.`id_item` = '{$id_item}'";

        $stmt1 = $cnn->query($sqlItems);

        if (!$stmt1) {
            throw new Exception("Ha surgido un error en la consulta, intentelo de nuevo", 1); 
        }
        $cnn-> close(); 
